...
EOF
    ...
    __print_lighthouse_functions_help
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Lighthouse github
        T=$(gettop)
        cd $T > /dev/null
        vendor/lighthouse/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/lighthouse/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
